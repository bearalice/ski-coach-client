...
export default function CoachDetail({ findCoach, deleteCoach, editCoach }) {
    ...
    useEffect(() => {
        async function fetchCoach() {
            const data = await fetch(baseURL + `/coaches/${id}`);
    ...
    useEffect(() => {
        async function fetchReviews() {
            const reviewData = await fetch(baseURL + `/${id}/reviews`);
    ...
    const addReview = async (review) => {
        const tmp = await fetch(baseURL + `/${id}/reviews`,
            {
                method: "POST",
    ...
    const edit_Coach = async (coach) => {
        ...
        const tmp = await fetch(baseURL + `/coaches/${id}`,
            {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
